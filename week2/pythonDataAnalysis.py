...
step[1:10:2]
# 1'den 10'a kadar ikişer ikişer git
...
# listeyi index gibi gör demek! ÖNEMMLİ
v[catch]
...
# .csv uzantılı bütün dizinleri yakala
all_files = glob.glob('C:/Users/rdn_a/Desktop/VBO/datasets' + "/*.csv")
all_files
...
"""
Eğer okuduğumuz verilerin tipleri aynı olsaydı (kolon sayısı ve tipleri)
bu verilerin hepsini tek bir dataframe haline getirebilirdir. 
Yani hepsini birleştirebilirdik.
"""
# all_df = pd.concat(file_list, axis=0, ignore_index=True)
#
# all_df = pd.concat(map(pd.read_csv, glob.glob('C:/Users/rdn_a/Desktop/VBO/datasets/*.csv')))
...
# Bütün hücrelerde eksik bilgi var mı yok mu diye bakar
df.isnull()
df.isnull().values
...
# 13. satırdan 18. satıra kadar getir
df[13:18]

# 0. index satırını sil
df.drop(0, axis=0).head()

# bu indexleri sil ve ilk 10 değeri göster
delete_indexes = [1, 3, 5, 7]
df.drop(delete_indexes, axis=0).head(10)

# yapılan işlemin kalıcı olmasını istiyorsak inplace=True
# axis=0 satırlara göre bak ve işlem yap
...
# PassengerId küçükten büyüğe doğru sırala
df.sort_values("PassengerId").head()

df = df.sort_values("PassengerId")

# PassengerId indexe çevirme
df.index = df["PassengerId"]
df

# passengerId değişkenini silme
df.drop("PassengerId", axis=1).head()
# PassengerId dışında kalan bütün değişkenleri seç demek
...
# atama sağdan sola atanır
df["PassengerId"] = df.index

# axis=1 sütunlara göre bak ve işlem yap
df.drop("PassengerId", axis=1, inplace=True)


# indexte olan kısmı değişkene çevir ve indexi sıfırla methodu
df.reset_index().head()
...
# değişkeni seri olarak algılanır
type(df["Age"])
# değişkeni dataframe olarak algılanır
type(df)

df[["Age"]].head()
# çift parantez ile seçersek dataframe olarak algılanır
type(df[["Age"]])
...
# olmayan bir isimlendirme yeni bir değişken oluşturur
df["Age2"] = df["Age"] ** 2
df["Age3"] = df["Age"] / df["Age2"]

df.head()

# sütun olarak bakarak Age3'ü sil ama bu kalıcı değil
df.drop("Age3", axis=1).head()

# listeye göre değişken silme
col_names = ["Age", "Embarked", "Ticket"]
...
# ~ değili anlamı katar
# df.columns.str.contains('Age') (içinde Age içerenleri) olanları getir demek (: tüm satırlar)
# ~df.columns.str.contains('Age') olmayanları getir demek (: tüm satırlar)
